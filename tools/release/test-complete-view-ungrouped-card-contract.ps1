...
if ($structure -notmatch '(?s)&-tree__section--ungrouped \{.*?--local-groupimport-easystud-ungrouped: #a76513;.*?identity-rail\(var\(--local-groupimport-easystud-icon-group\)\).*?&\.is-expanded:focus-within \{.*?card-focus-context' -or
...
if ($styles -notmatch '(?s)\.local-groupimport-easystud-tree__section--ungrouped \{.*?--local-groupimport-easystud-ungrouped: #a76513;.*?border-left-color: var\(--local-groupimport-easystud-ungrouped-rail\);' -or
