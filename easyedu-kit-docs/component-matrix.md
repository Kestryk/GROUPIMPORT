...
| Loading and navigation skeleton | `templates/manage.mustache`, `index.php`, `scss/components/_layout.scss`, `scss/views/_mass-import.scss` | `components/_loading.scss`, `components/_navigation-skeleton.scss`, `docs/components/navigation-skeleton.md` | done | K3 compact Navigation Skeleton, Guide cue, one-row RF1 consumer density, logical card `border-inline-start` accents and principal-container `border-block-start` accents, RTL, reduced motion and forced colors are integrated on every rendered EasyStud view with real navigation; lifecycle and real navigation remain plugin-owned. |
